...
import { useCreateSubmissionDraftMutation } from '@/features/donor-screening/hooks/mutations';
...
export default function SubmissionCreate() {
  const { formID } = useLocalSearchParams<SubmissionCreateSearchParams>();
  const router = useRouter();

  const { mutateAsync: create, isPending, isSuccess, error } = useCreateSubmissionDraftMutation();

  useEffect(() => {
    if (formID && !isSuccess) {
      create({ formID }).then((submission) => {
        router.replace(`/donor-screening/submission/${submission.id}`);
      });
    }
  }, [formID, create, router, isSuccess]);

  if (!formID) {
    return <Redirect href={'/donor-screening'} />;
  }
      ...
      <VStack space="xl" className="flex-1 items-center justify-center px-6">
        ...
        <VStack space="sm" className="items-center">
          <Heading size="xl" className="text-center">
            Failed to Begin Screening
          </Heading>
          <Text size="md" className="text-center text-typography-500">
            We couldn&apos;t process your screening. Please try again for a few minutes.
          </Text>
        </VStack>
