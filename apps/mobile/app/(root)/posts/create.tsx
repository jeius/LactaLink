...
import { useCreatePostMutation } from '@/features/feed/hooks/mutations';
...
export default function PostCreate() {
  ...
  const { mutateAsync: createPostAsync, isPending } = useCreatePostMutation();

  async function onSubmit(data: PostSchema) {
    const toastID = 'create-post-toast';

    toast.loading('Submitting post...', { id: toastID, cancel: null });
    await createPostAsync({ data }).then(
      () => {
        toast.success('Post created successfully!', { id: toastID });
        router.dismissTo('/feed');
        setTimeout(() => deleteSavedFormData('post-create'), 200);
      },
      (err) => {
        toast.error(`Error post: ${extractErrorMessage(err)}`, { id: toastID });
      }
    );
  }
