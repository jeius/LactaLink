...
import { useInfinitePosts } from '@/features/feed/hooks/queries';
  ...
  return (
    <SafeArea className="items-stretch">
      <AnimatedFlashList
        ...
        ListHeaderComponent={<ListHeader isLoading={isLoading} />}
        ListFooterComponent={<ListFooter hasNextPage={hasNextPage} />}
  ...
  return (
    <VStack space="sm">
      <HStack
        space="sm"
        style={shadow.sm}
        className="border border-outline-200 bg-background-0 p-4"
      >
        <Link asChild push href={'/posts/create'}>
...
function ListFooter({ hasNextPage }: { hasNextPage: boolean }) {
