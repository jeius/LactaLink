...
const baseStyle = tva({
  base: 'bg-background-50 relative flex-1 items-center justify-center',
});
...
export function Protected({ className, ...props }: SafeAreaViewProps) {
  ...
  if (isLoading || !user || !session) {
    return (
      <SafeAreaView className={baseStyle()}>
        <Spinner color={getHexColor(theme, 'primary', 500)} size={'large'} />
      </SafeAreaView>
    );
  }

  return <SafeAreaView className={baseStyle({ class: className })} {...props} />;
}
