...
export function DonationDetails({ data }: DetailsProps<Donation>) {
...
export function RequestDetails({ data }: DetailsProps<Request>) {
  const { themeColors } = useTheme();
...
export function OrganizationDetails({ data }: DetailsProps<Hospital | MilkBank>) {
  const { themeColors } = useTheme();
  const iconFillColor = themeColors.tertiary[50];
  const iconStrokeColor = themeColors.tertiary[700];

  const { avatar, displayName, receivedTransactions, inventory } = data;
