import React, { useCallback, useEffect, useMemo, useState } from 'react';
import { FieldPath, FieldValues, useWatch } from 'react-hook-form';

import { ComboboxField } from '@/components/form-fields/ComboboxField';
import { BaseFieldProps } from '@/components/form-fields/types';
import { extractID } from '@lactalink/utilities/extractors';
import debounce from 'lodash/debounce';
import { useCityMunicipality, useInfiniteCities } from '../../hooks/queries';

type Props<
  TFieldValues extends FieldValues = FieldValues,
  TName extends FieldPath<TFieldValues> = FieldPath<TFieldValues>,
> = Omit<BaseFieldProps<TFieldValues, TName>, 'error'> & {
  placeholder?: string;
  provinceID?: string;
};

export function CitySelectField<
  TFieldValues extends FieldValues = FieldValues,
  TName extends FieldPath<TFieldValues> = FieldPath<TFieldValues>,
>({
  placeholder = 'Select a city/municipality...',
  provinceID,
  ...props
}: Props<TFieldValues, TName>) {
  const [inputValue, setInputValue] = useState('');
  const [search, setSearch] = useState('');

  const debouncedSetSearch = useMemo(() => debounce(setSearch, 300), [setSearch]);

  const currentCity = useWatch({ name: props.name, control: props.control });

  const { data: doc, ...query } = useCityMunicipality(extractID(currentCity));

  const { data: docs, ...listQuery } = useInfiniteCities(search, provinceID);

  const error = query.error || listQuery.error;

  const handleTextChange = useCallback(
    (text: string) => {
      setInputValue(text);
      debouncedSetSearch(text);
    },
    [debouncedSetSearch]
  );

  useEffect(() => debouncedSetSearch.cancel, [debouncedSetSearch]);

  return (
    <ComboboxField
      {...props}
      error={error}
      items={docs}
      transformItem={(item) => ({ value: item.id, label: item.name })}
      triggerInputProps={{
        placeholder: placeholder,
        value: query.isLoading ? 'Loading city/municipality...' : doc?.name,
        isLoading: query.isLoading,
      }}
      comboboxProps={{
        listProps: listQuery,
        searchInputProps: {
          showSearchIcon: true,
          value: inputValue,
          onChangeText: handleTextChange,
          placeholder: 'Search city/municipality here...',
        },
      }}
    />
  );
}
