import {
  PACKAGE_MAP_PIN_BLUE,
  PACKAGE_MAP_PIN_PINK,
  USER_MAP_PIN,
...
const DESTINATION_PROPOSED_SVG: RNMarkerSvg = {
  width: 48,
  height: 48,
  svgString: PACKAGE_MAP_PIN_BLUE,
};
...
export function useTransactionMapMarkers(
  ...
  return useMemo<RNMarker[]>(() => {
    ...
    if (deliveryDetail && transaction.status === 'CONFIRMED') {
      ...
      const { method, address } = deliveryPlan;
      const addressDoc = extractCollection(address);
      const coordinates = addressDoc?.coordinates;
      ...
      if (coordinates) {
        markers.push({
          ...
          coordinate: pointToLatLng(coordinates),
          title: `${DELIVERY_OPTIONS[method].label} Location`,
          iconSvg: DESTINATION_PROPOSED_SVG,
          anchor: { x: 0.32, y: 1 },
          infoWindowAnchor: { x: 0.32, y: 0 },
          zIndex: 1,
          draggable: false,
