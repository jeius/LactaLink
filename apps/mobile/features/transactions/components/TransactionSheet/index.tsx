...
import { TransactionCTA } from '@/components/buttons/TransactionCTA';
...
export function TransactionSheet({ transaction: data, ...props }: TransactionSheetProps) {
  ...
  function renderContent() {
    switch (status.value) {
      case PENDING: {
        if (!latestProposal) {
          return <Propose transactionID={data} />;
        }
        return (
          <ProposalInformation
...
function extractData(data: Transaction) {
  return {
